...
const PrintServiceModal = ({ open, onClose, business, user, data }) => {
  const printRef = useRef();

  const { discount, date, products } = data;
  const subtotal = products?.reduce((acc, product) => acc + product.price * product.qty, 0);
...
export default PrintServiceModal;
